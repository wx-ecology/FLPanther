...
data <- read.csv("./JWM_resubmission/Final_Variable_All_New.csv")
...
data <- data %>% dplyr::arrange (Den_ID) %>%
  mutate(Ave_Wet_scale = scale(Ave_Wet),
         Perc_Tree_scale = scale(Perc_Tree))
```
...
## STEP 2 RESULT - KEEP mod 4 (no Veg_Class)
lrtest(cand.models.2[[4]], cand.models.2[[2]])
...
# STEP 3: 
cand.models.3 <- list()
cand.models.3[[1]] <- cand.models.2[[4]]
cand.models.3[[2]] <- update(cand.models.2[[4]],  . ~ . - Perc_Tree)
cand.models.3[[3]] <- update(cand.models.2[[4]],  . ~ . - Ave_Wet)
...
# clogit(Used ~ Fire_Cat + Perc_Tree + Veg_Class + Ave_Wet + L.dist2build + strata(Den_ID), data = data, method='efron')
# 
# data0 <- data
# data <- data %>% filter ((Den_ID != "61") & (Den_ID != "94") & (Den_ID != "95A"))
# data$Veg_Class <- droplevels(data$Veg_Class)
cand.models.0 <- list()
cand.models.0[[1]] <- clogit(Used ~ Fire_Cat:Perc_Tree + Perc_Tree + Fire_Cat + Veg_Class + strata(Den_ID), data = data, method='efron') #forage impotant theory
cand.models.0[[2]] <- clogit(Used ~ Fire_Cat:Veg_Class + Perc_Tree + Fire_Cat + Veg_Class + strata(Den_ID), data = data, method='efron') #forage impotant theory
cand.models.0[[3]] <- clogit(Used ~ Perc_Tree + Fire_Cat + Veg_Class + Ave_Wet + strata(Den_ID), data = data, method='efron') #forage impotant theory
cand.models.0[[4]] <- clogit(Used ~ Perc_Tree + Fire_Cat + Veg_Class + strata(Den_ID), data = data, method='efron') #
cand.models.0[[5]] <- clogit(Used ~ Fire_Cat + Veg_Class + strata(Den_ID), data = data, method='efron') #forage impotant theory
cand.models.0[[6]] <- clogit(Used ~ Fire_Cat + Perc_Tree + Veg_Class  + strata(Den_ID), data = data, method='efron') #hidden important theory
cand.models.0[[7]] <- clogit(Used ~ Fire_Cat + Perc_Tree  + strata(Den_ID), data = data, method='efron') #hidden important theory
cand.models.0[[8]] <- clogit(Used ~ Fire_Cat + strata(Den_ID), data = data, method='efron') #raw
...
```{r}
set.seed(17)
data.10 <- data.full %>% group_by (Den_ID) %>% filter( Used == 0 ) %>% sample_n(10)
